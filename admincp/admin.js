$(function(){
        ...
        function goPost(postid,fid)
        {
               
             $.ajax({
                type: "POST",
                url: 'fb.php',
                data: {'id':postid,'uid':fid},
                success: function(data){
                       if(data.st=="error")
                         {
                            ...
                            if(data.nid=="0")
                                {
                             $('.msgs').append('<div class="alert alert-error"><button type="button" class="close" data-dismiss="alert">&times;</button> تم النشر بنجاح.</div>');
                             $('.msgs').fadeIn(2000); 
